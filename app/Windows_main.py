...
def find_unity_persistent_path(product_name: str) -> Union[Path, None]:
    ...
    try:
        if system == "Windows":
            ...
            print(f"  [PathFinder] Error: OS '{system}' not supported for automatic search. This tool is configured for Windows.")
            return None
...
def SimulationGraphics(simulation_name):
    """
    Generates a set of graphs from the data of a specific simulation.

    The main change is in the last graphing section, which now generates
    a separate graph for each organism type to avoid scaling issues
    in visualization.
    """
    if not simulation_name:
    ...
    try:
        print(f"Step 2: Ensuring graphics folder exists...")
    ...
    print(f"Step 3: Verifying and reading CSV file...")
    ...
    print(f"Step 4: Processing data...")
    try:
        ...
        df = df[~df["Timestamp_str"].str.lower().isin(['0', ''])].copy()
    ...
    known_columns = {"Timestamp", "Timestamp_str", "FPS", "RealTime", "SimulatedTime", "DeltaTime", "FrameCount", "Pausado", "Paused"}
    organism_columns = sorted([col for col in df.columns if col not in known_columns and col != "Organism count"])
    print(f"  Identified organism columns: {organism_columns}")
    print(f"Step 5: Generating graphs...")
    ...
    # --- Graph 3: Organism Counts (Grouped) ---
    if organism_columns:
        plt.figure(figsize=(12, 6))
        ...
        plt.tight_layout()
        try:
            plt.savefig(str(output_folder / "organism_counts_grouped.png"))
            plot_generated_count += 1
        except Exception as e:
            print(f"Error saving organism_counts_grouped.png: {e}")
        plt.close()

    # --- Graph 4: FPS vs Total Organism Count ---
    if "Organism count" in df.columns and "FPS" in df.columns:
        df_groupable = df.dropna(subset=["Organism count", "FPS"])
        if not df_groupable.empty and pd.api.types.is_numeric_dtype(df_groupable["Organism count"]):
    ...
    # --- CORRECTED SECTION: Individual Organism Count Graphs with Exponential Fit ---
    if "SimulatedTime" in df.columns and organism_columns:
        time_data_full = df["SimulatedTime"]

        # Loop to create a dedicated graph for each organism
        for col in organism_columns:
            print(f"  Generating graph for organism: {col}")
            if not (col in df.columns and pd.api.types.is_numeric_dtype(df[col]) and not df[col].isnull().all()):
                print(f"  -> Warning: Skipping '{col}' due to missing, non-numeric, or null data.")
                continue

            # Create a new figure for this specific organism
            plt.figure(figsize=(12, 7))

            valid_indices = df[col].notna() & time_data_full.notna()
            time_data_unsorted = time_data_full[valid_indices].values
            organism_data_unsorted = df.loc[valid_indices, col].values

            if len(time_data_unsorted) < 2:
                print(f"  -> Warning: Not enough data points ({len(time_data_unsorted)}) to fit curve for '{col}'.")
                plt.close()
                continue

            # --- FIX: Sort data by time before plotting ---
            sort_indices = np.argsort(time_data_unsorted)
            time_data_clean = time_data_unsorted[sort_indices]
            organism_data_clean = organism_data_unsorted[sort_indices]
            # --- END OF FIX ---

            # Plot the actual data
            plt.plot(time_data_clean, organism_data_clean, label=f"Actual Count of {col}", marker=".", linestyle="-", alpha=0.8)

            # Attempt exponential curve fitting
            try:
                # --- Improved logic for initial parameter estimation (p0) ---
                initial_a = organism_data_clean[0] if organism_data_clean[0] > 0 else 1.0
                initial_b = 0.01  # Default value

                if len(time_data_clean) > 1 and organism_data_clean[-1] > initial_a:
                    time_diff = time_data_clean[-1] - time_data_clean[0]
                    # Avoid division by zero and logarithm of non-positive values
                    if time_diff > 1e-6 and initial_a > 0:
                        ratio = organism_data_clean[-1] / initial_a
                        initial_b = np.log(ratio) / time_diff
                
                if not np.isfinite(initial_b): # Safety check
                    initial_b = 0.01
                ...
                params, _ = curve_fit(
                    ...
                    p0=p0, 
                    maxfev=10000,
                    bounds=([0, -np.inf], [np.inf, np.inf]) # Restrict 'a' to be non-negative
                )
                a_fit, b_fit = params

                time_fit = np.linspace(time_data_clean.min(), time_data_clean.max(), 200)
                ...
                label_fit = f"Exp Fit: a={a_fit:.2f}, b={b_fit:.3f}, R²={r_squared:.2f}"
                plt.plot(time_fit, organism_fit, label=label_fit, linestyle="--")

            except (RuntimeError, ValueError) as e:
                print(f"  -> Warning: Could not fit exponential curve for '{col}'. ({e})")

            # Finalize and save the individual graph
            plt.title(f"Count and Exponential Fit for {col} ({simulation_name})")
            ...
            try:
                safe_col_name = "".join(c for c in col if c.isalnum() or c in (' ', '_')).rstrip().replace(' ', '_')
                save_path = output_folder / f"{safe_col_name}_vs_simulated_time_fit.png"
                plt.savefig(str(save_path))
                plot_generated_count += 1
                print(f"  -> Graph saved as: '{save_path.name}'")
            except Exception as e:
                print(f"  -> Error saving graph for '{col}': {e}")
            
            plt.close() # Close the figure to free up memory before the next iteration

    else:
        print("'SimulatedTime' or organism columns not found to generate fit graphs.")
...
DELIMITER = "%|%"
try:
    APP_DATA_DIR = Path.home() / "Documents" / "UnityMicroVerseData"
...
def on_closing():
    ...
    if messagebox.askokcancel(
        ...
        message="Are you sure you want to exit the MicroVerse?",
        icon='question'
        ):
...
def open_graphs_folder(simulation_name):
    ...
    try:
        ...
        if platform.system() == "Windows":
            ...
            messagebox.showerror("Unsupported Operation", f"Opening folders is currently only supported on Windows. Current OS: {platform.system()}")
    except Exception as e:
...
def get_build_target_and_executable(project_path: Union[str, Path, None]) -> Tuple[Union[str, None], Union[str, None]]:
    ...
    if system == "Windows":
        ...
        print(f"Warning: Unsupported OS '{system}'. Build target determination is for Windows only.")
        return None, None
    ...
    if exe_path.exists():
        try:
            ...
            if platform.system() == "Windows":
                 ...
                 messagebox.showerror("Unsupported Operation", f"Launching executables is only supported on Windows. Current OS: {platform.system()}")
                 update_status(f"Error: Cannot launch on {platform.system()}.")
                 return
    ...
    if not UNITY_EXECUTABLE:
        ...
        try:
            ...
            if f"{os.path.sep}{req_ver}{os.path.sep}" in str(unity_exe_path):
                 ...
                 parent_dir_name = unity_exe_path.parent.parent.name
...
main_window.title("MicroVerse v1.0")
initial_width=1050
initial_height=700
...
def handle_unity_download_click():
    ...
    if system_os == "Windows":
        ...
        if 'main_window' in globals() and main_window and main_window.winfo_exists():
             messagebox.showinfo("Information", "Unity Editor download assistance is configured for Windows.", parent=main_window)
        else:
             print("Unity Editor download assistance is configured for Windows.")
        return
    instructions = (
...
about_btn = ctk.CTkButton(sidebar_frame, text="About",
                          command=lambda: messagebox.showinfo("About", "MicroVerse v1.0.\n\nAuthors:\nIván Cáceres S.\nTobías Guerrero Ch."),
                          font=APP_FONT,
...
ctk.CTkLabel(header_frame, text="MicroVerse", font=TITLE_FONT, anchor="center").grid(row=0, column=0, pady=(0, 10))
